...
The predicate (`isNetlifyHost`, `$lib/indexability.ts`) is **"this is a netlify.app host"**, never "this is not 29navy.com". The second form is the one that must be removed at cutover and kills the site when nobody remembers; this one cannot match the client's domain, so it never needs removing, and it keeps protecting the netlify.app mirror _after_ launch, when duplicate-content risk is highest. It suffix-matches a dot-prefixed label rather than using `includes`, so `netlify.app.example.com` — someone else's host — cannot silently noindex a real domain. A `netlify.toml` header rule was considered and rejected for the same reason the prerender was: Netlify header rules cannot be scoped by host, so `for = "/*"` would carry the noindex onto `29navy.com` after cutover.

Six assertions were shown failing against unfixed source before anything changed, with nine passing in the same run as the control — the same instruments on the production host, where they can and do pass. Confirmed after merge on the host that was actually exposed: `29-navy.netlify.app/robots.txt` is now `User-agent: *` / `Disallow: /` with no Sitemap line, `sitemap.xml` returns an empty `urlset`, and `/robots.txt` carries `x-robots-tag: noindex, nofollow`.

**A reading that would have been a false pass, caught only by looking twice.** On the deploy preview, `/` answered with `x-robots-tag: noindex` and `/contact` with _two_ values — `noindex, nofollow` plus a bare `noindex`. The bare one is **Netlify's own**, added automatically to deploy previews and branch deploys. Treating `/`'s header there as proof that the hook covers prerendered pages would have been vouching for a header this change never set, on the one host type that was never at risk. It does not cover them: prerendered pages are served by the CDN without running `hooks.server.ts`, and their bytes are identical on both hosts, so `Disallow: /` is what fences those. The production host settles it — one `x-robots-tag` on `/robots.txt`, none on `/`, and none at all before this change, which is what Netlify's preview-only header had been masking.
