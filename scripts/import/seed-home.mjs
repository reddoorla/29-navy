 ...
 *   node --env-file=.env scripts/import/seed-home.mjs --apply    # writes
 *
...
async function main() {
  ...
  if (!writeToken)
    throw new Error("--apply needs PRISMIC_WRITE_TOKEN (use `node --env-file=.env …`)");
  ...
  console.log(`\n✔ done — ${assets.size} asset(s), ${plan.length} document(s).`);
